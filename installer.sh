...
if [ -d "$FLAKE_DIR/.git" ]; then
	if [ -n "$(git -C "$FLAKE_DIR" status --porcelain --untracked-files=all 2>/dev/null)" ]; then
		die "$FLAKE_DIR is a git repo with uncommitted/untracked files.
...
sed -i \
	...
	"$FLAKE_DIR/disko.nix"
grep -q CHANGE_ME "$FLAKE_DIR/disko.nix" && die "disko.nix still contains a CHANGE_ME placeholder"
