...
| **Packages** | `brave-origin alacritty herdr claude-code git gh jujutsu devenv sccache shellcheck gitleaks uv nh nixd statix nixfmt ripgrep fd jq comma appflowy` + nix-ld. `jjui eza fzf` moved to Home — each has a `programs.*` block that configures it, so declaring it twice was two copies of one fact |
