...
let
  ...
  # Which walls Codex does not have, taken from what the two modules actually
  # wire rather than from memory. modules/nixos/agent-wiring.nix carries the why;
  # the short version is that this paragraph used to be hand-written here and was
  # wrong within three commits, in the file Codex reads as its rules.
  codexMissingGuards = lib.subtractLists osConfig.agents.wiredGuards.codex osConfig.agents.wiredGuards.claude;

  # Throws rather than rendering a shorter list. A wall that is absent AND
  # unmentioned is worse than one that is merely absent, so a guard added without
  # a note fails the build instead of quietly narrowing this section.
  guardNote =
    n:
    osConfig.agents.guardNotes.${n} or (throw (
      "modules/nixos/agent-guards.nix has no absenceNote for the guard ${n}, "
      + "so AGENTS.md would silently omit a wall Codex does not have."
    ));

  # No leading indentation: Nix strips the common indent from the literal parts of
  # an indented string only, so an interpolation lands at whatever column the
  # stripping leaves and its own newlines are inserted verbatim.
  codexMissingWalls = lib.concatMapStringsSep "\n" (n: "- ${guardNote n}") codexMissingGuards;
  ...
  codexAgentsMd = pkgs.writeText "AGENTS.md" (
    ''
      ...
      **And some are thinner than Claude's.** The file rule reads `apply_patch`'s
      own text, so editing a generated file through the SHELL (`sed -i`, `cat >`) is
      not walled off for you at all — Claude has an auto-mode classifier covering
      that route and you do not. The spill rule is best-effort: it keys on a path
      shape that has not been seen in a real Codex payload, so treat it as yours to
      follow rather than as enforced.

      **And some are not wired for you at all.** The list below is generated from
      what modules/nixos/codex.nix actually attaches, subtracted from what
      claude.nix does, so it cannot describe a wall you have or miss one you lack:

      ${codexMissingWalls}

      Everything not named above — the deny list, the TUI rules, the commit gate,
      the law-1 command shapes and the stop switch — applies to you in full.
